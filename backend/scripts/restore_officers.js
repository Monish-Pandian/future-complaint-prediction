...
async function restoreOfficers() {
  try {
    ...
    console.log(`[Restore] Loaded ${centroids.length} community area centroids`);

    // Step 1: Remove demo officers (OFF-1001 through OFF-1005, OFF-SANJAY)
    const demoOfficerIds = ['OFF-1001', 'OFF-1002', 'OFF-1003', 'OFF-1004', 'OFF-1005', 'OFF-SANJAY'];
    const demoDeleteResult = await Officer.deleteMany({ officerId: { $in: demoOfficerIds } });
    console.log(`[Restore] Removed ${demoDeleteResult.deletedCount} demo officers`);

    // Also remove their user accounts
    const demoEmails = [
      'marcus.vance@civic.gov',
      'elena.rostova@civic.gov',
      'kwame.mensah@civic.gov',
      'carlos.rivera@civic.gov',
      'sarah.jenkins@civic.gov',
      'sanjay@gmail.com'
    ];
    const userDeleteResult = await User.deleteMany({ email: { $in: demoEmails } });
    console.log(`[Restore] Removed ${userDeleteResult.deletedCount} demo user accounts`);

    // Step 2: Seed 500 officers
    const existingCount = await Officer.countDocuments();
    console.log(`[Restore] Existing officers after demo removal: ${existingCount}`);

    const targetCount = 500;
    const additionalNeeded = Math.max(0, targetCount - existingCount);
    console.log(`[Restore] Need to create: ${additionalNeeded} additional officers`);

    if (additionalNeeded === 0) {
      console.log('[Restore] Already have 500+ officers. Skipping seed.');
    ...
    for (const dept of DEPARTMENTS) {
      ...
      for (let i = 0; i < deptCount; i++) {
        ...
        const userEmail = `${officer.officerId.toLowerCase()}@civic.gov`;
        ...
        officer.userId = userDoc._id;

        officersToCreate.push(officer);
      }
    }

    console.log(`[Restore] Prepared ${officersToCreate.length} officers for insertion`);
    ...
    for (let i = 0; i < officersToCreate.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        console.log(`[Restore] Inserted ${inserted}/${officersToCreate.length} officers...`);
      } catch (err) {
        if (err.writeErrors) {
          inserted += batch.length - err.writeErrors.length;
          console.log(`[Restore] Batch partial success: ${batch.length - err.writeErrors.length}/${batch.length}`);
        } else {
          console.error(`[Restore] Batch error: ${err.message}`);
        }
      }
    }
    ...
    console.log(`\n[Restore] Complete!`);
    console.log(`[Restore] Officers before: ${existingCount}`);
    console.log(`[Restore] Officers added: ${inserted}`);
    console.log(`[Restore] Total officers now: ${finalCount}`);
    ...
    console.log('\n[Restore] Department Distribution:');
    ...
    console.log('\n[Restore] Community Area Distribution (top 15):');
    ...
    console.log('\n[Restore] Workload & Availability Stats:');
    console.log(JSON.stringify(workloadStats[0] || {}, null, 2));

    const uniqueOfficerIds = await Officer.distinct('officerId');
    console.log(`\n[Restore] Unique officerIds: ${uniqueOfficerIds.length}`);
    
    const uniqueEmployeeCodes = await Officer.distinct('employeeCode');
    console.log(`[Restore] Unique employeeCodes: ${uniqueEmployeeCodes.length}`);

    // Verify all officers have valid homeCommunityArea
    const invalidAreas = await Officer.find({ 
      $or: [
        { homeCommunityArea: null },
        { homeCommunityArea: '' },
        { homeCommunityArea: 'null' }
      ]
    }).lean();
    console.log(`\n[Restore] Officers with invalid homeCommunityArea: ${invalidAreas.length}`);

    // Verify centroid mapping
    const allOfficers = await Officer.find({}).lean();
    let validCentroids = 0;
    for (const o of allOfficers) {
      if (centroidMap.has(o.homeCommunityArea)) validCentroids++;
    }
    console.log(`[Restore] Officers with valid centroid mapping: ${validCentroids}/${allOfficers.length}`);
